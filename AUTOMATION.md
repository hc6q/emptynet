...
13. Open a pull request into `main` so EMPTYNET CI validates asset preparation, JavaScript syntax and the server health check.
14. Inspect CI. If it fails, fix the branch and re-run validation. Never merge a failing build.
15. When CI passes, squash-merge the pull request into `main` with a descriptive message.
16. The production host tracks `main`, so the merge becomes the next automatic production deployment.
17. Report exactly what changed, the PR/commit, validation result and any deployment issue.
